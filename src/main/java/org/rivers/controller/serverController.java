...
import org.rivers.service.RiverCenter;
import org.rivers.util.Folder;
...
public class serverController {
	...
	public String getStatus() {
		...
		data.put("instances", ((HashMap<String, HashMap<String, HashMap<String, Object>>>) tmp.get("data")).size());
		...
		for (String path : GlobalConfig.riverUrls.split(",")) {
			try {
				String str = HttpClient.sendGet("http://" + path + "/search.doaction",
						"ac=getStatus&code=" + getCode("getStatus"));
				JSONObject jr = JSONObject.fromObject(str);
				version = JSONObject.fromObject(jr.get("info")).getString("VERSION");
	...
	public String reloadInstanceConfig(@RequestParam String instance) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		ZKUtil.upload(
				this.getClass().getProtectionDomain().getCodeSource().getLocation().getPath() + "datas/" + instance,
				GlobalConfig.zkInstancePath);
			...
			for (Iterator it = socketlist.iterator(); it.hasNext();) {
				...
				if (sqlmapname.equals(socket.element("name").getTextTrim())) {
					...
					for (Iterator socket_item = socket_contentlist.iterator(); socket_item.hasNext();) {
						...
						if (resourcemap.get(socket_content.getName()) != null
								&& resourcemap.get(socket_content.getName()) != "") {
							...
							if (socket_content.getText() != resourcemap.get(socket_content.getName())
								...
								System.out.println("===== update Element <socket> name: " + socket_content.getName()
										+ "：" + socket_content.getText() + "--->"
										+ resourcemap.get(socket_content.getName()));
								socket_content.setText(resourcemap.get(socket_content.getName()));
							}
							resourcemap.remove(socket_content.getName());

						} else if (resourcemap.get(socket_content.getName()) == null
								|| resourcemap.get(socket_content.getName()) == "") {
							// 删除<socket>节点下<name>/<type>等节点
							System.out.println(
									"===== delete Element <socket>  name:" + socket_content.getName() + " 节点 ======");
					...
					if (resourcemap.size() > 0) {
						...
						for (Map.Entry<String, String> entry : resourcemap.entrySet()) {
							System.out.println("=====  Element <socket> Nodename: " + sqlmapname + "  add :"
...
//			System.out.println(doc.asXML());
			...
			if (isexist) {
				...
				for (String path : GlobalConfig.riverUrls.split(",")) {
					String str = HttpClient.sendGet("http://" + path + "/search.doaction",
							"ac=addResource&type=" + sqlmaptype + "&code=" + getCode("updateResourceXml")) + "&socket="
							+ jsonStr;
				}
			}
			res.put("data", "success");
	...
	@ResponseBody
	public String setInstanceConfig(@RequestParam String instance, @RequestParam String data) {
		String _res = "100";
		try {
			data = data.trim();
	...
	@ResponseBody
	public String createInstanceConfig(@RequestParam String instance, @RequestParam String data,
	...
	@ResponseBody
	public String InstancesAction(@RequestParam String action, @RequestParam String instance, @RequestParam String ip) {
		String _res = "100";
		switch (action) {
	...
	@ResponseBody
	public String resume_job(@RequestParam String ip, @RequestParam String instancetype, @RequestParam String instance,
			@RequestParam String type) {
		return HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=resumeInstance&instance=" + instance + "&code=" + getCode("resumeInstance") + "&type=" + type);
	}

	@RequestMapping(value = "/stop_job", method = RequestMethod.GET, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String stop_job(@RequestParam String ip, @RequestParam String instancetype, @RequestParam String instance,
			@RequestParam String type) {
		return HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=stopInstance&instance=" + instance + "&code=" + getCode("stopInstance") + "&type=" + type);
	}

	@RequestMapping(value = "/reload_config", method = RequestMethod.GET, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String reload_config(@RequestParam String ip, @RequestParam String instancetype,
			@RequestParam String instance) {
		return HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=reloadConfig&instance=" + instance + "&code=" + getCode("reloadConfig"));
	}

	@RequestMapping(value = "/run_job", method = RequestMethod.GET, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String run_job(@RequestParam String ip, @RequestParam String jobtype, @RequestParam String instance) {
		return HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=runNow&jobtype=" + jobtype + "&instance=" + instance + "&code=" + getCode("runNow"));
	}
	...
	private String saveNodeConfig(String ip, String type, String k, String v) {
		String _res = "400";
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=setNodeConfig&code=" + getCode("setNodeConfig") + "&type=" + type + "&k=" + k + "&v=" + v);
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String pushInstanceToNode(String instance) {
		...
		String ip = getMaster();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=addInstance&code=" + getCode("addInstance") + "&instance=" + instance);
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String stopHttpReaderFromNode(String ip) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=stopHttpReaderServiceService&code=" + getCode("stopHttpReaderServiceService"));
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String startHttpReaderFromNode(String ip) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=startHttpReaderServiceService&code=" + getCode("startHttpReaderServiceService"));
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String getNodeConfig(String ip) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=getNodeConfig&code=" + getCode("getNodeConfig"));
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String stopSearcherFromNode(String ip) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=stopSearcherService&code=" + getCode("stopSearcherService"));
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String startSearcherFromNode(String ip) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=startSearcherService&code=" + getCode("startSearcherService"));
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String runCode(String codes, String ip) {
		...
		if (ip.length() > 5) {
			try {
				String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
						"ac=runCode&code=" + getCode("runCode") + "&script=" + URLEncoder.encode(codes, "utf-8"));
				res.put("data", str);
				_res = JSONArray.fromObject(res).toString();
			} catch (Exception e) {
	...
	private String resetInstanceState(String instance, String ip) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=resetInstanceState&code=" + getCode("resetInstanceState") + "&instance=" + instance);
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
	...
	private String removeInstanceFromNode(String instance, String ip) {
		...
		HashMap<String, Object> res = new HashMap<String, Object>();
		String str = HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=deleteInstanceData&code=" + getCode("deleteInstanceData") + "&instance=" + instance);
		str += HttpClient.sendGet("http://" + ip + "/search.doaction",
				"ac=removeInstance&code=" + getCode("removeInstance") + "&instance=" + instance);
		res.put("data", str);
		_res = JSONArray.fromObject(res).toString();
		return _res;
	}
		...
		for (String path : GlobalConfig.riverUrls.split(",")) {
			try {
				String str = HttpClient.sendGet("http://" + path + "/search.doaction",
						"ac=getStatus&code=" + getCode("getStatus"));
				JSONObject jr = JSONObject.fromObject(str);
				if (JSONObject.fromObject(jr.get("info")).getString("NODE_TYPE").toLowerCase().equals("master")) {
					_res = path;
					break;
				}
			} catch (Exception e) {
